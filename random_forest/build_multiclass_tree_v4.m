...
function tree = build_multiclass_tree_v4(opts,data,images)
    tree_queue = 1;
    delete_index = [];
...
%                     [feature1, offset1] = sample_node_data_v7(opts,WI,data,images,1);
%                     [feature2, offset2] = sample_node_data_v7(opts,WI,data,images,1);
                    for func_type = 1:opts.numfunctypes
%                         [feature, offset] = ...
%                             sample_node_data_v7(opts,WI,data,images,func_type,feature1,feature2,offset1,offset2);
                        [feature, offset, channel] = ...
                            sample_node_data_v7(opts,WI,data,images,func_type);
...
%                     fprintf('Computing for node: %d\n',tree_index);
                    ...
                    if ~isempty(bestT) %not a leaf node
                        tree(tree_index).test = [double(best_offset(best_offset_idx,:)), double(bestfunctype), double(bestT), bestchannel];
