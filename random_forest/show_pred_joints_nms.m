...
function show_pred_joints_nms(opts,forest,showGT,pred_joints)
...
%           shoulder_h  = plot(j(1,[6,7]),j(2,[6,7]),'b-','linewidth',5);
            ...
            for c = 3:(opts.numclasses+1)
                X = sample_nms_conf(opts,dist(:,:,c-2));
                subplot(3,3,c)
                h_pred(c-2) = imagesc(dist(:,:,c-2)); axis image
                hold on
                h_maxima(c-2) = plot(X(:,1),X(:,2),'bo','markerfacecolor','w');
                title(plot_title{c})
            end 
        else
...
%             set(shoulder_h,'xdata',j(1,[6,7]),'ydata',j(2,[6,7]));
            ...
            for c = 3:(opts.numclasses+1)
                ...
                X = sample_nms_conf(opts,dist(:,:,c-2));
                if size(X,1)>1
                    set(h_maxima(c-2),'xdata',X(1:2,1),'ydata',X(1:2,2));
                else
                    set(h_maxima(c-2),'xdata',X(1,1),'ydata',X(1,2));
                end
            end
