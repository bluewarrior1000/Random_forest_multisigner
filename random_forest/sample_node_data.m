...
function feature = sample_node_data(opts,window_index)
    ...
    for i = 1:num_windows_at_node
        ...
        if image_idx_prev ~= image_idx || i==num_windows_at_node
            if i==num_windows_at_node
                ...
                img = imread(sprintf('%sim%04.0f.png',opts.image_dir,image_idx));
            else
                load_idx = img_idx_sort(last_loaded_idx:(i-1));
                img = imread(sprintf('%sim%04.0f.png',opts.image_dir,image_idx_prev));
            end
            ...
            if func_type==1
                ...
                for window = 1:length(load_idx)
                    ...
                    feature.feature_val(load_idx(window)) = mean(img(y,x,:),3);
                end
            else
               feature.offsetx1 = offset(1);
                feature.offsety1 = offset(2);
                ...
                for window = 1:length(load_idx)
                    ...
                    feature.feature_val(load_idx(window)) = func([mean(img(y1,x1,:),3),mean(img(y2,x2,:),3)]);
                end
            end
