...
opts.numsampleimages    = 500;              %number of images to sample durring training
...
opts.trainingset        = opts.valid_frames(300:20:floor(length(opts.valid_frames)*0.6));   %indicies of the images we wish to use as training data 
opts.testingsetstepsize = 8;
opts.testingset         = opts.valid_frames((floor(length(opts.valid_frames)*0.6)+1):opts.testingsetstepsize:37999);         %indicies of the images we wish to use as training data  
...
%% options for forest
opts.forest.numtrees     = 8; %number of trees to use in forest
opts.forest.maxdepth     = 16; %maximum depth to grow each tree
