function mcc_wrapper_tomas(sarg)
    ...
    video_type = {'silhouette','lab','tomas'};
    ...
    %set video type here
    opts.frames_dir         = sprintf('/nobackup/scsjc/one_signer/frames/%s/',video_type{vt});
    opts.forest_dir         = sprintf('/nobackup/scsjc/one_signer/forests/%s/',video_type{vt});
    opts.data_dir           = '/nobackup/scsjc/data/';
    
%     opts.frames_dir         = sprintf('./frames/%s/',video_type{vt});
%     opts.forest_dir         = sprintf('./forests/%s/',video_type{vt});
%     opts.data_dir           = '../data/';
    
    
    
    %load some options
    opts.windowwidth        = window_width(ww);
    ...
    %load in images
    load(sprintf('%svideo%d/images%d.mat',opts.frames_dir,opts.video_num,opts.windowwidth));
