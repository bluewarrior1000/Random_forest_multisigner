...
opts.trainingset        = opts.valid_frames(1:4:floor(length(opts.valid_frames)*0.6));   %indicies of the images we wish to use as training data 
opts.testingsetstepsize = 8;
opts.testingset         = opts.valid_frames((floor(length(opts.valid_frames)*0.6)+1):opts.testingsetstepsize:37000);         %indicies of the images we wish to use as training data  
...
opts.forest.maxdepth     = 32; %maximum depth to grow each tree
