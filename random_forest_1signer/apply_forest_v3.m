...
%v3 - splits image_set into chunks to be more memory efficient
function idxp = apply_forest_v3(opts, forest, image_set, visualise,  data)

    if nargin <= 4
        unique_img_idx = 1:size(image_set,4);
    else
        unique_img_idx = unique(data.img_index);
    end
    
    data_y = uint16(opts.boundingbox(2):(opts.boundingbox(2) + opts.boundingbox(4) - 1));
    data_x = uint16(opts.boundingbox(1):(opts.boundingbox(1) + opts.boundingbox(3) - 1));
    [X,Y] = meshgrid(data_x,data_y);
    
    %chunk up the data into small portions to reduce the memory requirments
    chunk_size = 50;
    num_chunks = mod(numel(unique_img_idx),chunk_size);
    for chunk = 1:num_chunks
        if chunk < num_chunks
            idx = (chunk_size*(chunk-1)+1):(chunk_size*(chunk-1) + chunk_size);
        else
            idx = (chunk_size*(chunk-1)+1):numel(unique_img_idx);
        end
        
        data_x = repmat(X(:),numel(idx),1);
        data_y = repmat(Y(:),numel(idx),1);
        data_img_index = repmat(uint16(1:numel(idx)),size(X(:),1),1);
        data_img_index = data_img_index(:);
        distrib_temp = uint8(zeros(numel(data_x),opts.numclasses,numel(forest)));
        
        %classify each pixel according to a weighted vote at leaf nodes
        for f = 1:numel(forest)
            tree = forest{f};
            pixel_index = cell(numel(tree),1);
