...
function points = sample_data_v4(opts, tree_num)
    %fix random seed
    if nargin<2
        tree_num = 0;
    end
...
%                 locs = opts.joint_patch(:,:,:,1+100*(sample_img_idx(i)-1)); %INDEX JUST USED FOR TESTING CODE
        ...
        if 0; %visualise the samples
            ...
            hold on
            plot(points.x(points.img_index(:)==find(opts.trainingset==sample_img_idx(i))), points.y(points.img_index(:)==find(opts.trainingset==sample_img_idx(i))),'r.')
            pause
        end
    end
