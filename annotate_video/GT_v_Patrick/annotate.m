...
if exist(data_filename,'file')
    ...
    joints = opts.joints;
    %normalise pose
%     joints2 = bsxfun(@minus,joints,joints(:,1,:));
%     joints2 = reshape(joints2,14,length(opts.testingset));
%     
%     %do clustering
%     andata1.centroids = vgg_kmeans(joints2,opts.seg.numposeclusters,'verbose',0);
%     andata1.joints = zeros(2,7,opts.seg.numposeclusters);
%     %find poses closest to cluster centroids
%     andata1.frameidx = zeros(1,opts.seg.numposeclusters);
%     [c_idx, c_dist] = vgg_nearest_neighbour(joints2,andata1.centroids);
%     joints = joints(:,:,c_idx);
